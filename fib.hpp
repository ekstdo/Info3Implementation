...
public: 
	...
	node(T inner) : node(inner, NULL, NULL, this, this, 0, false) {
		this->inner = inner;
	}
	...
	node<T>* insert_sibling(T val){ // inserts another sibling into the list
...
public:
	...
	node<T>* insert(T el){
		...
		if (list == NULL){
			...
			node<T>* elem = list->insert_sibling(el);
			ret = elem;
		}
		n++;
	...
	void del_min(){
		...
		if (min == list) // minimale Element entfernen und die Kinder davon in die Liste einzufügen
			list = min->next_or_child();

		
		min->isolate_adopt(); /* this->print(0); */
		delete min;
		if (list == NULL)
			return;
		list->remove_parent();
